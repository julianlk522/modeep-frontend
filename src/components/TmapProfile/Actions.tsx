...
import { ERR_STATUS_RANGE_START, TMAP_ABOUT_ENDPOINT, TMAP_EMAIL_ENDPOINT, TMAP_PFP_ENDPOINT } from '../../constants'
...
export default function Actions(props: Props) {
	...
	const [show_delete_pic_modal, set_show_delete_pic_modal] = useState<boolean>(false)
	...
	async function handle_new_pic(e: Event) {
		...
		data.append('pic', new_pic)

		const new_pic_resp = await fetch_with_handle_redirect(TMAP_PFP_ENDPOINT, {
			method: 'POST',
	...
	async function handle_delete_pic(e: MouseEvent) {
		...
		const delete_resp = await fetch_with_handle_redirect(TMAP_PFP_ENDPOINT, {
			method: 'DELETE',
			headers: { Authorization: `Bearer ${token}` },
		})
	...
	async function update_about() {
		...
		const edit_about_resp = await fetch_with_handle_redirect(TMAP_ABOUT_ENDPOINT, {
			method: 'PUT',
			headers: {
	...
	async function update_email() {
		...
		const edit_email_resp = await fetch_with_handle_redirect(TMAP_EMAIL_ENDPOINT, {
			method: 'PUT',
			headers: {
				'Content-Type': 'application/json',
				Authorization: `Bearer ${token}`,
	...
	return (
		<div id="profile-actions">
			...
			<input id="new-pic-upload" type="file" accept={'image/*'} hidden onChange={handle_new_pic} />
			{pic_url ? (
				<button
				...
				>
					<img src="../../../delete.svg" height={22} width={22} alt="Remove profile pic" />
				</button>
			) : null}
			...
			{is_editing_about ? (
				...
				>
					<img src="../../../confirm.svg" height={20} width={20} alt="Save Changes" />
				</button>
			) : (
				<button
					id="edit-about-btn"
				...
				>
					<img src="../../../edit.svg" height={18} width={18} alt="Toggle about section editing" />
				</button>
			)}
			{is_editing_email ? (
				<button
				...
				>
					<img src="../../../email.svg" height={20} width={20} alt="Toggle email editing" />
				</button>
			)}
